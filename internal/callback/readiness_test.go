...
func TestDNSReadinessDoesNotWaitForAnUnstartedAppOrHideOtherFailures(t *testing.T) {
	for _, result := range []map[string]any{
		...
		{"outcome": "http_response", "status": 530, "dead_tunnel_signature": "error code: 1016"},
	} {
...
func TestTunnelReadinessRetries1033UntilOriginAnswers(t *testing.T) {
	var probes atomic.Int32
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if probes.Add(1) == 1 {
			w.Write([]byte(`{"probe_state":"unreachable","last_probe_result":{"outcome":"http_response","status":530,"dead_tunnel_signature":"error code: 1033"}}`))
			return
		}
		w.Write([]byte(`{"probe_state":"answered","last_probe_result":{"outcome":"http_response","status":502}}`))
	}))
	defer srv.Close()
	state, err := New(srv.URL, Options{}).probeResolved(context.Background(), time.Second, time.Millisecond)
	if err != nil || !state.Answered() || probes.Load() != 2 {
		t.Fatalf("state=%+v err=%v probes=%d", state, err, probes.Load())
	}
}

func TestPersistent1033IsBoundedAndNotCalledDNSFailure(t *testing.T) {
	c, _ := sandbox(t, map[string]any{"probe_state": "unreachable", "last_probe_result": map[string]any{"outcome": "http_response", "status": 530, "dead_tunnel_signature": "error code: 1033"}})
	_, err := c.probeResolved(context.Background(), 30*time.Millisecond, time.Millisecond)
	if !errors.Is(err, ErrTunnelNotReady) || !errors.Is(err, context.DeadlineExceeded) || errors.Is(err, ErrDNSNotReady) {
		t.Fatalf("got %v", err)
	}
}

func TestTunnelReadinessCancellation(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	var probes atomic.Int32
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		probes.Add(1)
